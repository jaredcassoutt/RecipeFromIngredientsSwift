...
// MARK: - TableView
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        //returns the number of cells that should be contianed in the UITableView
        return LocalData.recipies.recipieList.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        //returns information for the UI of the UITableView such as the title, number of likes, image, and missing ingredients
        ...
        cell.menuMissingIngredients.text = "Missing Ingredients: \(LocalData.recipies.recipieList[indexPath.row].missedIngredientCount)"
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        cell.background.backgroundColor = UIColor.tertiarySystemGroupedBackground
        LocalData.selected.recipeID = LocalData.recipies.recipieList[indexPath.row].id
        self.performSegue(withIdentifier: "Item Segue", sender: self)
        cell.background.backgroundColor = UIColor.systemBackground
    }
...
//MARK: - Helper Methods
    ...
    func loadRecipies(ingredients: String, loadNumber: Int) {
        ...
        Networking().fetchRecipes(ingredients: ingredients, loadNumber: loadNumber, success: { (decodedData) in
            LocalData.recipies.recipieList = [Recipie]()
            for recipie in decodedData {
                LocalData.recipies.recipieList.append(Recipie(id: recipie.id, title: recipie.title, image: recipie.image, missedIngredientCount: recipie.missedIngredientCount, likes: recipie.likes, usedIngredientCount: recipie.usedIngredientCount))
            }
